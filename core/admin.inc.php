...
function addAdmin(){
	$arr=$_POST;
	$arr['userPassword']=md5($_POST['password']);
	if(insert("mind_admin",$arr)){
...
function getAllAdmin(){
	...
	$sql="select userId,userName,email from mind_admin ";
	$rows=fetchAll($sql);
	return $rows;
}
function getAdminByPage($page,$pageSize=2){
	$sql="select * from mind_admin";
	global $totalRows;
	...
	$sql="select userId,userName,email from mind_admin limit {$offset},{$pageSize}";
	$rows=fetchAll($sql);
	// print_r($rows);
	return $rows;
}
...
function editAdmin($id){
	...
	if(update("mind_admin", $arr,"id={$id}")){
 ...
 */
function delUser($id){
	$sql="select face from mind_user where userId=".$id;
	$row=fetchOne($sql);
	$face=$row['face'];
	if(file_exists("../uploads/".$face)){
		unlink("../uploads/".$face);
	}
	if(delete("mind_user","userId={$id}")){
