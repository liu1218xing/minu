...
function editmCate($id){
	...
	$where="id={$id}";
	$res=update("mind_cate",$arr,$where);
	// var_dump($arr);
	...
	if($res&&$pid){
		...
		$mes="<p>编辑成功!</p><a href='listmCate.php' target='mainFrame'>查看商品列表</a>";
	}else{
	if(is_array($uploadFiles)&&$uploadFiles){
		foreach($uploadFiles as $uploadFile){
			if(file_exists("../image_800/".$uploadFile['name'])){
		...
		$mes="<p>编辑失败!</p><a href='listmCate.php' target='mainFrame'>重新编辑</a>";
